...
public abstract class GradleSettingsWriter {
	...
	private void writePluginMapping(IndentingWriter writer, PluginMapping pluginMapping) {
		...
		Assert.notNull(dependency.getVersion(), "Dependency version must not be null");
		Assert.notNull(dependency.getVersion().getValue(), "Dependency version value must not be null");
